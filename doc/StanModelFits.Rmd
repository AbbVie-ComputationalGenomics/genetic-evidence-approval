...
```{r label=RunningStanBoth2013, include=FALSE, cache=TRUE}
...
for (i in 1:length(stan_res_list)) {
  pred_res[[i]] <- cbind(predict_gene_mesh(ensembl_ids = associated_genes, mesh_terms = indications, association_table = gene_trait_association_table, similarity = average_similarity, gene_table = gene_properties, top_mesh = mesh_properties, stan_res= stan_res_list[[i]], default_time = 13903.5, conf = 0.95, cutoff=0.5), data.frame(Model=names(stan_res_list)[i]))
}
...
for (i in 1:length(stan_res_list)) {
  pred_res[[i]] <- cbind(predict_gene_mesh(ensembl_ids = target_genes, mesh_terms = indications, association_table = gene_trait_association_table, similarity = average_similarity, gene_table = gene_properties, top_mesh = mesh_properties, stan_res= stan_res_list[[i]], default_time = NA, conf = 0.95, cutoff=0.5), data.frame(Model=names(stan_res_list)[i]))
}
